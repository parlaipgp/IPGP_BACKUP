...
cmt_name = "CMT_Hanetal_Hollywood";
...
% corrected for sign match
% Diagonal components
Mrr = sin(2 * delta) * sin(lambda);
Mpp = 0.5 * (-sin(2 * delta) * sin(lambda) + 2 * sin(delta) * cos(lambda) * sin(2 * phi) - sin(2 * delta) * sin(lambda) * cos(2 * phi));
Mtt = 0.5 * (-sin(2 * delta) * sin(lambda) - 2 * sin(delta) * cos(lambda) * sin(2 * phi) + sin(2 * delta) * sin(lambda) * cos(2 * phi));

% Cross-components
Mrt = -(cos(delta) * cos(lambda) * cos(phi) + cos(2 * delta) * sin(lambda) * sin(phi));
Mrp = -(-cos(delta) * cos(lambda) * sin(phi) + cos(2 * delta) * sin(lambda) * cos(phi));
Mtp = -(0.5 * sin(2 * delta) * sin(lambda) * sin(2 * phi) + sin(delta) * cos(lambda) * cos(2 * phi));

% check Mtt Mpp from Han et all 2015
Mtt_Mpp_Diff_1 = -(2*sin(delta) * cos(lambda) * sin(2*phi) - sin(2*delta) * sin(lambda) * cos(2*phi))
Mtt_Mpp_Diff_2 = Mtt - Mpp
...
for ii = 1:n_CMT
    ...
    fprintf(fid,'PDE  2011 03 11 05 46 24.00 %8.2f %8.2f %6.2f mb Ms CMT by Han et al-%d\n', ...
        lat, lon, depth, ii);
    fprintf(fid,'event name:     S D R  Mw %d %d %d %1.2f\n',strike,dip,rake,Mw);
    fprintf(fid,'time shift:     8.2\n');
    fprintf(fid,'half duration:  %f\n',HD_sub);
    fprintf(fid,'latitude:       %8.4f\n', lat);
    fprintf(fid,'longitude:      %8.4f\n', lon);
    fprintf(fid,'depth:          %6.2f\n', depth);
    fprintf(fid,'Mrr:      %13.5fe+23\n', Mrr*M0_sub / 1e23);
    fprintf(fid,'Mtt:      %13.5fe+23\n', Mtt*M0_sub / 1e23);
    fprintf(fid,'Mpp:      %13.5fe+23\n', Mpp*M0_sub / 1e23);
    fprintf(fid,'Mrt:      %13.5fe+23\n', Mrt*M0_sub / 1e23);
    fprintf(fid,'Mrp:      %13.5fe+23\n', Mrp*M0_sub / 1e23);
    fprintf(fid,'Mtp:      %13.5fe+23\n', Mtp*M0_sub / 1e23);
end
